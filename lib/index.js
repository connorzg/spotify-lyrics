...
window.app = new Vue({
  ...
  methods: {
    ...
    setView: function(viewName) {
      ...
      switch (viewName) {
        case 'lyrics-viewer':
          this.resizeWindow([null, 640])
          break;
        case 'detecting-spotify':
          this.resizeWindow([540, 420])
          break;
      }
    },
    ...
    resizeWindow: function(size) {
      try {
        ...
        let targetSize = size.slice()
        let winSize = window.getSize()

        if (targetSize[0] == null || targetSize[0] > winSize[0])
          targetSize[0] = winSize[0]

        if (targetSize[1] == null || targetSize[1] > winSize[1])
          targetSize[1] = winSize[1]

        window.setSize(targetSize[0], targetSize[1], true)
      } catch (e) {
        console.log(e)
      }
    },
